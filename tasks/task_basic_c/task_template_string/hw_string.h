#pragma once /* исключает множественное включение библиотек */
#include <iostream>

/*
 * && - это rvalue-ссылка. Она указывает, что объект временный, не имеет постоянного адреса и скоро будет уничтожен.
 * & - это lvalue-ссылка. Она указывает, что объект постоянный и имеет постоянный адрес.
 * Чтобы объект можно было рассматривать как rvalue, нам нужно применить std::move.
 * */

/*
 * ПРАВИЛО ПЯТИ
 *
 * Оно гласит: если вы определяете хотя бы один из нижеописанных методов, то надо определить и все оставшиеся
 *
 * 1. Деструктор.
 *
 * Вызывается при уничтожении объекта. Не принимает и не возвращает никаких значений.
 *
 * 2. Конструктор копирования.
 *
 * Вызывается, когда создается новый объект как копия другого объекта того же класса.
 * Принимает константную ссылку на объект, из которого будет сделана копия.
 *
 * 3. Оператор копирующего присваивания.
 *
 * Вызывается при присваивании уже существующему объекту некоторого класса значения другого существующего объекта того же класса
 * Возвращает ссылку на текущий объект
 *
 * Копирование нужно для создания независимой копии, чтобы каждый объект обладал собственной независимой копией.
 * Если её не делать, что оба объекта будут указывать на одну и ту же область памяти, что при уничтожении одного объекта
 * непременно приведёт к ошибкам и двойному освобождению
 *
 * 4. Конструктор перемещения.
 *
 * Метод класса, который вызывается тогда, когда создается новый объект, перемещая ресурсы из объекта, который
 * скоро будет уничтожен
 * Принимает rvalue-ссылку на объект, из которого будут перемещены ресурсы
 * noexcept нужен для того, чтобы контейнеры могли безопасно перемещать элементы для управления памятью. Без этого ключевого
 * слова конструктор может бросить исключение, что может привести к потере данных.
 *
 * 5. Оператор перемещающего присваивания
 *
 * Метод класса, который вызывается тогда, когда уже существующему объекту класса присваиваются ресурсы из другого
 * существующего объекта того же класса, который скоро будет уничтожен
 * Принимает rvalue-ссылку на объект, из которого будут перемещены ресурсы
 * Возвращает ссылку на текущий объект
 *
 * */
